...
export function JobProfileEditor({
  ...
  // Apply a single expected_score to ALL competencies in a module
  function setModuleScore(moduleId: string, score: number) {
    const comps = competenciesByModule[moduleId] ?? []
    if (comps.length === 0) return
    setSettings(prev => {
      const next = { ...prev }
      for (const c of comps) {
        next[c.id] = { ...(next[c.id] ?? { weight: 1, expected_score: 70 }), expected_score: score }
      }
      return next
    })
  }

  // Apply a single weight to ALL competencies in a module
  function setModuleWeight(moduleId: string, weight: number) {
    const comps = competenciesByModule[moduleId] ?? []
    if (comps.length === 0) return
    setSettings(prev => {
      const next = { ...prev }
      for (const c of comps) {
        next[c.id] = { ...(next[c.id] ?? { weight: 1, expected_score: 70 }), weight }
      }
      return next
    })
  }
                  ...
                  <div className="flex items-center gap-3" onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center gap-1.5">
                      ...
                      <Input
                        ...
                        placeholder="—"
                        value={(() => {
                          const vals = comps.map(c => getCompetencySetting(c.id).weight)
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className="text-xs text-muted-foreground whitespace-nowrap">Attendu</span>
                      <Input
                        type="number"
                        min={0}
                        max={100}
                        ...
                        placeholder="—"
                        value={(() => {
                          const vals = comps.map(c => getCompetencySetting(c.id).expected_score)
                          return vals.length > 0 && vals.every(v => v === vals[0]) ? vals[0] : ''
                        })()}
                        onChange={(e) => {
                          const v = Math.min(100, Math.max(0, parseInt(e.target.value) || 0))
                          setModuleScore(module.id, v)
                        }}
                      />
